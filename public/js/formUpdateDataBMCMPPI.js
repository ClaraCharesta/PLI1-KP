document.addEventListener('DOMContentLoaded', () => {
  ...
  function loadActivities(category) {
    const currentValue = activitiesSelect.dataset.current || '';

    activitiesSelect.innerHTML = '<option value="">-- Pilih Aktivitas --</option>';

    if (!activitiesMap[category]) return;
  ...
  categorySelect.addEventListener('change', function () {
    activitiesSelect.dataset.current = '';
  ...
  if (btnCancel && cancelModal) {
    ...
    if (confirmCancelBtn) {
      confirmCancelBtn.addEventListener('click', () => {
        ...
        window.location.href = '/dataBMCMPPI';
      });
    }
